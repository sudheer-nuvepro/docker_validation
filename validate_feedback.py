...
class Activity:
    ...
    def testcase_check_docker_image(self, test_object):
        ...
        expected_result = "Docker image exists with name 'feedback_management'"
        actual = "Docker image not found with name 'feedback_management'"
        marks = 5
        test_object.update_pre_result(testcase_description, expected_result)
        try:
            result = subprocess.getoutput("docker images -q feedback_management")
            if result.strip():
                actual = "Docker image exists with name 'feedback_management'"
                return test_object.update_result(1, expected_result, actual, testcase_description, "N/A", marks)
            else:
                return test_object.update_result(0, expected_result, actual, testcase_description, "N/A", marks)
        except Exception:
